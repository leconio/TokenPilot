...
import { chatTargets, eventFor, mutableProperties, type ChatEnvironment } from "./chat-common.js";
import { requireAiContext } from "./context.js";
import { AiProviderRequestError, requestProvider } from "./provider-transport.js";
import type { AiChatAttempt, AiChatInput, AiChatResult } from "./types.js";

export async function executeChat<T>(
  input: AiChatInput,
  ...
  const targets = chatTargets(route, input, false);
      ...
      try {
        ...
        const failure =
          error instanceof AiProviderRequestError
            ? error
            : new AiProviderRequestError("Model service request failed.", null, "failure", true);
        lastError = failure;
        const latencyMs = Math.max(0, Math.round(performance.now() - started));
        const final =
          !failure.retryable ||
          (targetIndex === targets.length - 1 && retry === maximumAttempts - 1);
        attempts.push({
          attemptId,
          attemptIndex,
          target,
