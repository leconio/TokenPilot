"""Asynchronous streaming chat execution."""
        ...
        for retry in range(connection.max_retries + 1):
            ...
            try:
                if adapter is None:
                    ...
                    async with client.provider_client.stream(
                        "POST",
                        url,
                        headers=headers,
                        json=body,
                ...
                if token is not None:
                    try:
                        await client.release_user_aiu_reservation(token, "stream was cancelled")
                    except Exception as error:
                        client.on_error(error)
                try:
                    await client.report_usage_events(events)
