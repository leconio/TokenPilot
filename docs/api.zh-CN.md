...
事件可以包含时间、请求与尝试标识、接入端和应用版本、调用连接、真实模型标识、虚拟模型、Token 与多模态用量、服务商上报的可选 `source_cost`、结果字段和类型化自定义字段。`source_cost` 包含非负十进制 `amount`、三位 `currency` 和 `is_estimated`。提示词、回复、消息、工具参数、cookie、认证头和服务商密钥会在持久接收前被拒绝或移除。
...
`PUT /models/:id/cost-rules` 会发布完整且有顺序的备用规则。每条规则包含名称、满足全部或任意条件、可选的 `fixed_amount`，以及零项或多项带实际 `amount_per_unit` 的 `rates`。空规则列表表示只统计调用端上报的金额。只有事件没有 `source_cost` 时才会计算这些规则，它不会改动 `/models/:id/aiu`。
