...
TokenPilot 提供严格的 HTTP API，用于接收用量、管理应用、查询统计以及向可信 LiteLLM 下发运行配置。模型请求仍直接发送给 LiteLLM，TokenPilot 不提供模型生成接口。
...
事件可以包含时间、请求与尝试标识、接入端和应用版本、LiteLLM 模型名称、虚拟模型、Token 与多模态用量、结果字段和类型化自定义字段。提示词、回复、消息、工具参数、cookie、认证头和服务商密钥会在持久接收前被拒绝或移除。
