-- Current empty-database baseline statement 0013.
-- Deterministically rolls every inserted 1m partial into a reduced-dimension daily partial.
CREATE MATERIALIZED VIEW usage_agg_1m_to_daily_mv
TO usage_agg_daily
AS
SELECT
    toDate(usage_agg_1m.bucket_start) AS event_date,
    toStartOfDay(usage_agg_1m.bucket_start) AS bucket_start,
    application_id,
    instance_id,
    environment,
    virtual_model,
...
GROUP BY
    ...
    toStartOfDay(usage_agg_1m.bucket_start),
    application_id,
    instance_id,
    environment,
    virtual_model,
