...
test("模型只需名称和 LiteLLM 模型名称，并可配置花费与 AIU", async ({ page }) => {
  ...
  await dialog.getByLabel("LiteLLM 模型名称").fill("anthropic/claude-sonnet-4-5");
  ...
  expect(mock.callsFor("POST", "/applications/support/models")[0]?.body).toEqual({
    name: "推理模型",
    litellm_tag: "anthropic/claude-sonnet-4-5",
  });
...
test("发布中心只展示服务端确认状态，并可查看原因后重新发布", async ({ page }) => {
  ...
  mock.runtimeStates.set("support", {
    state: "rejected",
    error: "默认模型在 LiteLLM 中不可用",
  });
  await mock.install(page);
  await page.goto("/apps/support/releases");

  await expect(page.getByText("未采用", { exact: true })).toBeVisible();
  await expect(page.getByText("默认模型在 LiteLLM 中不可用", { exact: true })).toBeVisible();
